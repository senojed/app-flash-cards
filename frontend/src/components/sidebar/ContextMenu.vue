<template>
  <div class="relative">
    <button @click.stop="open = !open" class="text-gray-600 hover:text-gray-300 px-1">⚙️</button>
    <div v-if="open" class="absolute right-0 top-5 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-30 w-36 py-1 text-xs">
      <button v-if="showSettings" @click="emit('settings'); open = false" class="w-full text-left px-3 py-2 hover:bg-gray-700 text-gray-300">⚙️ Nastavení</button>
...
<script setup>
import { ref } from 'vue'
defineProps({ showReset: { type: Boolean, default: true }, showSettings: { type: Boolean, default: false } })
const emit = defineEmits(['settings', 'rename', 'reset', 'delete'])
